...
always @(*) begin
    case (state)
        S0:next_state =  end_cnt_state ? PAOMA_LED:FLOW_LED;
        S1:next_state =  end_cnt_state ? SHANSHUO:PAOMA_LED; 
        S2:next_state =  end_cnt_state ? FLOW_LED:SHANSHUO;   
...
always @(posedge clk or negedge rst) begin
    if(!rst)begin
        ...
        flow_led <= {flow_led_led[2:0],flow_led[3]};
    else if(c_state == PAOMA_LED & end_cnt_led)
        paoma_led <= {flow_led_led[2:0],~flow_led[3]};
    else if(c_state == PAOMA_LED & end_cnt_led)
        shanshuo <= ~shanshuo;
end
//输出
